...
const filterExercises = (array) => {
  ...
  searchList = array.filter(
    (exercise) => exercise.muscle_group === searchExercise
  )
...
const deleteExercise = (exercise) => {
  ...
  fetch(`http://localhost:3000/exercises/${exercise.id}`, options).then(
    (_resp) => document.getElementById(exercise.id).remove()
  )
}

const createRandomWorkout = (muscleGroup) => {
  subGroupObject[muscleGroup].forEach((subGroup) => {
    const subGroupExercises = searchList.filter(
      (exercise) => exercise.sub_group === subGroup
    )
